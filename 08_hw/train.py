...
def predict_seq2seq2(net, src_sentence, src_vocab, tgt_vocab, num_steps,
                    device, save_attention_weights=False):
    """Predict for sequence to sequence."""
    # Set `net` to eval mode for inference
    net.eval()
    ...
    for _ in range(num_steps):
        ...
        dec_X = torch.multinomial(input=prob, num_samples=1)
        dec_X = dec_X.view(1, -1)
...
def main():
    ...
    for rep in range(3):
        ...
        for eng in engs:
            ...
            translation2 = predict_seq2seq2(net, eng, src_vocab, tgt_vocab, num_steps, device)
            print(f'Argmax: {eng} => {translation}')
            print(f'Random: {eng} => {translation2}')
